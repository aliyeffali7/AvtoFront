...
export default function CustomerDetailClient() {
  ...
  useEffect(() => {
    ...
    getCustomer(parseInt(id))
      .then(res => { setCustomer(res.data); setNotes(res.data.notes ?? '') })
  ...
  if (!customer) {
    return <div className="p-8 text-center text-gray-500">Müştəri tapılmadı.</div>
  }
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 px-6 py-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  {(customer.car_brand || customer.car_model) && (
                    <p className="text-sm text-gray-600 mt-1">{[customer.car_brand, customer.car_model, customer.car_year].filter(Boolean).join(' ')}</p>
                  )}
                  {(customer.car_plate || customer.plates.length > 0) && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {customer.car_plate ? (
                        <span className="px-2.5 py-1 rounded-lg bg-gray-100 text-sm font-mono text-gray-700 font-semibold">{customer.car_plate}</span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => setEditOpen(true)} className="p-2.5 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                  </button>
                  <button onClick={() => setDeleteConfirm(true)} className="p-2.5 rounded-xl border border-red-200 text-red-500 hover:bg-red-50 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              ...
              <div className="grid grid-cols-3 gap-3 mt-5 pt-5 border-t border-gray-100">
                ...
                <div>
                  ...
                  <p className={`text-lg font-bold ${hasDebt ? 'text-red-600' : 'text-gray-400'}`}>{formatCurrency(customer.total_debt)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400 mb-0.5">Son ziyarət</p>
                  <p className="text-sm font-medium text-gray-700">
                    {customer.last_visit ? new Date(customer.last_visit).toLocaleDateString('az-AZ', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Sifarişlər ({customer.orders.length})</h2>
                {customer.orders.length > 0 && (
                  <button
                    onClick={() => printCustomerPDF(customer, business)}
                    className="flex items-center gap-1.5 text-xs font-medium text-green-700 bg-green-50 hover:bg-green-100 border border-green-200 px-3 py-1.5 rounded-xl transition-colors"
                  >
                ...
                <div className="flex flex-col gap-3">
                  {customer.orders.map(order => (
                    <div key={order.id} className="bg-white rounded-2xl border border-gray-200 hover:border-blue-300 hover:shadow-sm transition-all flex items-center gap-2 pr-3">
                      <Link to={`/business/orders/${order.id}`} className="flex-1 px-5 py-4 flex items-center justify-between gap-4 min-w-0">
                        <div className="flex-1 min-w-0">
                          ...
                          {order.mileage != null && <p className="text-xs text-gray-400 mt-0.5">{order.mileage.toLocaleString()} km</p>}
                        </div>
                      ...
                      <button onClick={() => printOrderPDF(order, business)} title="PDF yüklə" className="p-2 rounded-xl text-green-600 hover:bg-green-50 border border-green-200 transition-colors shrink-0">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                      </button>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 px-5 py-5">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Qeydlər</p>
                {!notesEditing && (
                  <button onClick={() => setNotesEditing(true)} className="text-xs text-blue-600 hover:text-blue-800 font-medium">
                    {notes ? 'Düzəlt' : 'Əlavə et'}
                  </button>
                )}
              </div>
              {notesEditing ? (
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={saveNotes} disabled={notesSaving} className="flex-1 btn-primary text-sm py-2">
                      {notesSaving ? 'Saxlanılır...' : 'Saxla'}
                    </button>
                    <button onClick={() => { setNotesEditing(false); setNotes(customer.notes ?? '') }} className="flex-1 btn-ghost text-sm py-2">Ləğv et</button>
                  </div>
                </div>
              ) : notes ? (
                <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{notes}</p>
              ) : (
                <p className="text-sm text-gray-400 italic">Qeyd yoxdur. Düzəlt düyməsini basın.</p>
              )}
            </div>
                ...
                <div className="flex flex-col gap-0">
                  {mileageHistory.map((order, i) => (
                    <div key={order.id} className={`flex items-center justify-between py-2.5 ${i < mileageHistory.length - 1 ? 'border-b border-gray-100' : ''}`}>
                      <div>
                        <p className="text-xs text-gray-400">{formatDate(order.created_at)}</p>
                        <p className="text-xs text-gray-500 mt-0.5 font-mono">{order.plate_number}</p>
                      </div>
                      <span className="text-sm font-semibold text-gray-800">{order.mileage!.toLocaleString()} km</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <EditCustomerDrawer
        ...
        onUpdated={updated => { setCustomer(updated); setNotes(updated.notes ?? '') }}
      />
